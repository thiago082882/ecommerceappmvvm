...
@Composable
fun GetCategories(navController: NavHostController,vm: AdminCategoryListViewModel = hiltViewModel()) {
    ...
    when(val response = vm.categoriesResponse) {
        ...
        is Resource.Success -> {
            AdminCategoryListContent(navController = navController, categories = response.data)
